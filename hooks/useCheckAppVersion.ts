...
export default function useCheckAppVersion() {
    ...
    useEffect(() => {
        hotUpdate.getCurrentVersion().then((value) => {
            console.log("Versão Atual", value);
            setVersion(value);
        });
    }, []);
    ...
    const onCheckVersion = async () => {
        ...
        const {
            ...
            downloadAndroidUrl,
            downloadIosUrl,
        } = await res.json().catch((error) => console.log(error));
        console.log("a");
        console.log(remoteVersion);
        if (remoteVersion > version) {
            const url =
                Platform.OS === "ios" ? downloadIosUrl : downloadAndroidUrl;
            Alert.alert("Há uma nova versão!", "Deseja atualizar agora?", [
